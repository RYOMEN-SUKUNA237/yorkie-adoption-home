-- =====================================================================
-- submit_application() - the public submission entry point
-- =====================================================================
-- Why this exists:
--
-- `applications` is deliberately write-only for the public: anon may
-- INSERT, but has no SELECT policy, so nobody can read submissions back.
-- That collides with needing to hand the applicant their reference number
-- (APP-0001) for the confirmation page, because `INSERT ... RETURNING`
-- requires SELECT permission on the new row and is refused by RLS.
--
-- Rather than weaken the read policy, submission goes through this
-- SECURITY DEFINER function. It returns only the reference, id and score
-- and never exposes any other row.
--
-- SECURITY DEFINER bypasses RLS, so every invariant the INSERT policy
-- enforced is re-checked here explicitly. Do not remove those checks.
-- =====================================================================
...
begin
  ...
  -- Mirrors the WITH CHECK on applications_public_insert.
  ...
  -- --- Bound the free-text and array inputs ----------------------------
  -- A SECURITY DEFINER function is reachable by anyone; keep a single
  -- submission from being used to store arbitrary bulk data.
  ...
  -- --- Insert ----------------------------------------------------------
  -- status / reviewed_at / reviewed_by / score / reference are set here
  -- and NOT taken from the payload, so a caller cannot pre-approve or
  -- self-score a submission. The scoring trigger fills in the rest.
  insert into public.applications (
    ...
    -- Only accept a puppy reference that actually exists.
...
comment on function public.submit_application(jsonb) is
  'Public submission entry point. SECURITY DEFINER so the applicant can be given their reference number without granting SELECT on applications. Re-checks every invariant the RLS INSERT policy enforces.';
